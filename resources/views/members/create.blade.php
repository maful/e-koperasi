    ...
    <div class="col-10 offset-md-1">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">{{ __('add') }} {{ __('menu.member') }}</h3>
            ...
            <form action="{{ route('members.store') }}" method="POST">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">{{ __('place_of_birth') }}</label>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="tanggal_lahir" id="datepicker" autocomplete="off" class="form-control{{ $errors->has('tanggal_lahir') ? ' is-invalid' : '' }}" value="{{ old('tanggal_lahir') }}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-controls-stacked">
                                    <label class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input{{ $errors->has('jenkel') ? ' is-invalid' : '' }}" name="jenkel" value="L"{!! old('jenkel') == 'L' ? ' checked=""' : '' !!}>
                                        <span class="custom-control-label">{{ __('male') }}</span>
                                    </label>
                                    <label class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input{{ $errors->has('jenkel') ? ' is-invalid' : '' }}" name="jenkel" value="P"{!! old('jenkel') == 'L' ? '' : ' checked=""' !!}>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control{{ $errors->has('agama') ? ' is-invalid' : '' }}" name="agama">
                                    ...
                                    @foreach (religions() as $religion)
                                        <option value="{{ $religion }}" {!! (old('agama') == $religion ? "selected=\"selected\"" : "") !!}>{{ $religion }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('agama'))
                ...
                <div class="card-footer text-right">
                    <button type="submit" class="btn btn-primary">{{ __('add') }} {{ __('menu.member') }}</button>
                </div>
            </form>
        </div>
    </div>
